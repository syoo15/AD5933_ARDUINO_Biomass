...
bool AD5933_Class::setNumofIncrement(unsigned int num) {
    ...
    // Write to register.
    return setByte(0x88, highByte) &&
           setByte(0x89, lowByte);
}
...
/*
 *  Function for performing full sweep with multi-frequency gain factor
 */

bool AD5933_Class::getComplexSweep(double *gainFactArr, double *pShiftArr, double impVal[], double phVal[]) {
   int ctrReg = getByte(0x80);
  ...
  while ( (getStatusReg() & 0x04) != 0x04) // Loop while if the entire sweep in not complete
  {
    ...
    rComp = impData[0] * 16 * 16 + impData[1];
    iComp = impData[2] * 16 * 16 + impData[3];
    //Serial.println(gainFactArr[t1]);

    //Serial.println(t1);
    double magSq = getMag(rComp, iComp);
    impVal[t1] = gainFactArr[t1] / magSq;
    //Serial.println(impVal[t1]);
    phVal[t1] = atan2(iComp, rComp) - pShiftArr[t1];
    //Serial.println(phVal[t1]);
...
bool AD5933_Class::getComplexData(int *rComp, int *iComp) {
